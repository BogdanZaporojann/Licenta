...
const AddChatMessage = ({ authorName, sendMessage, getRealtimeSocketMessage }) => {
    ...
    const handleClickCloseSmileArea = () => {
        debugger
        setTimeout(() => {
    ...
    return (
        <div onClick={handleClickCloseSmileArea} className={style.bottomContainer}>
            <div className={style.smile}>
                <div className={style.emoji}>
                    <img src={icon} alt="" onClick={() => setIsOpen(!isOpen)} />

                    {isOpen && (
                        <div onClick={handleClickSmileArea} className={style.emojies}>
